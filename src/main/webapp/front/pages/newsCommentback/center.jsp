	...
	<body style="background: #EEEEEE;">
					...
					<form class="layui-form" lay-filter="myForm">
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<select name="commentbackTypes" id="commentbackTypes">
                                    <option v-for="item in commentbackTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
								</select>
							</div>
						</div>
		...
		<script>
            ...
            layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function () {
                ...
                http.request(`newsCommentback/session`, 'get', {}, function (data) {
                    // 表单赋值
				...
				//类型的查询
			   function commentbackTypesSelect() {
				   //填充下拉框选项
				   http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=commentback_types", "GET", {}, (res) => {
					   if(res.code == 0){
						   vue.commentbackTypesList = res.data.list;
					   }
				   });
			   }
